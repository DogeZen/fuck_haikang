...
def record_device_num(driver):
    ...
    time.sleep(1)
    device_num=driver.find_element_by_xpath('//input[@ng-model = "oSettingBasicInfo.szSerialNo"]').get_attribute('value')[-9:]
...
if __name__ == '__main__':
    ...
    if is_admin():
        ...
        print('''
                请确认以下事项:
                1.如果连接不上摄像头就断网再试
                2.画遮挡区域时不要移动鼠标直到配置完成
                3.配置过程中不要打开excel，否则会导致无法写入
                4.配置时保持chromedriver.exe打开
                ''')
